...
(defn- fetch-app-access-token
  ...
  (let [response (http/post feishu-app-access-token-url
                           {:form-params {:app_id (sso.settings/feishu-auth-app-id)
                                        :app_secret (sso.settings/feishu-auth-app-secret)}
                            :as :json})]
...
(mu/defn feishu-auth-fetch-or-create-user! :- (ms/InstanceOf :model/User)
  "Create a new Feishu Auth user."
  [user-info]
  ...
  (let [{:keys [name email department_names]} (:data user-info)
        ...
        existing-user (t2/select-one [:model/User :id :email :last_login :first_name :last_name]
    ...
    (doseq [dept-name department_names]
      (let [group-name (str/join "/" ["智谱" dept-name])
            existing-group (t2/select-one :model/PermissionsGroup :name group-name)
            group (or existing-group
                     (t2/insert-returning-instance! :model/PermissionsGroup
                                                  :name group-name))]
        ;; Add user to group if not already a member
        (when-not (t2/exists? :model/PermissionsGroupMembership
                             :group_id (:id group)
                             :user_id (:id user))
...
(defn do-feishu-auth
  ...
  (when-not (sso.settings/feishu-auth-enabled)
